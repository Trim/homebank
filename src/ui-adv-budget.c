...
static gboolean ui_adv_bud_model_row_filter_with_headers (GtkTreeModel *model, GtkTreeIter *iter, gpointer user_data);
...
static void ui_adv_bud_category_merge_full_filled (GtkWidget *source, gpointer user_data);
...
gchar *label_source, *label_delete;
	...
	categories = gtk_tree_model_filter_new(budget, NULL);
	gtk_tree_model_filter_set_visible_func(GTK_TREE_MODEL_FILTER(categories),
		ui_adv_bud_model_row_filter_with_headers, data, NULL);
	...
	if (gtk_tree_selection_get_selected(selection, &filter, &filter_iter))
	{
		...
		// If currently selected row is a leaf, take its parent
		gtk_tree_model_get (budget, &iter_source,
			ADVBUD_CATEGORY_KEY, &item_key,
			-1);

		// No control as the merge button is only enabled when selected row is valid to merge
		merge_source = da_cat_get(item_key);
		...
		DB( g_print("[ui_adv_bud] open sub-dialog to merge category: %s\n", merge_source->name) );
		...
		// Signals to enable Apply button
		g_signal_connect (data->COMBO_merge_target, "changed", G_CALLBACK(ui_adv_bud_category_merge_full_filled), (gpointer)data);
		...
		gint target_key;
			...
			gtk_tree_model_get (budget, &iter,
				ADVBUD_CATEGORY_KEY, &target_key,
				-1);
			...
			{
				da_cat_remove(merge_source->key);
